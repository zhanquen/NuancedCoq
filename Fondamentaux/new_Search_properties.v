...
Notation C1 := (VCG.OStar_choice O1).

Variable G_choice : C1. (* c1 *)

Definition O1_winners (a1s : A1s) := (proj1_sig (G_choice (VCG.oStars_ne0 o10 a1s))).1.1.
(* .1.1 stands for winners_uniq *)
...
Section Relationsbreaking.

Notation C2 := OStar.OStar_choice.

Variable c2 : C2.

Definition O2_winners_b a2s := OStar.oStar_chosen c2 a2s.

Definition O2_prices_b a2s: k.-tuple P := (**)
  map_tuple (inord \o (fun i => OStar.price_chosen c2 a2s i)) (O2_winners_b a2s).

Definition O2_outcome_b a2s := (O2_winners_b a2s, O2_prices_b a2s, a2s).

Definition M2_b : A2s -> O2 := O2_outcome_b.

Definition m2_b : mech.type n := mech.new M2_b.

Notation v2_b := v2.

Notation is_winner2_b := is_winner2.

Definition p2_b : prefs.type m2_b :=
  prefs.new v2 
            (fun i (o2 : mech.O m2_b) => 
               let: s := slot_of i o2.1.1 in
               if is_winner2 o2.1.1 i then v2 i * 'ctr_s - tnth o2.1.2 s 
               else 0)
            v2.

Lemma o2_in_ofRi2_b a2s : O2_outcome_b a2s \in VCG.oStars (fRi a2s).
Proof.
rewrite inE andTb.
apply/forallP => o1.
apply/implyP => _.
rewrite /O2_outcome_b/O2_winners_b/=.
move: (OStar.le_welfare_o_oStar_chosen c2 a2s o1.1.1).
rewrite /OStar.max_welfare /OStar.welfare -!bidSum_slot => bx.
rewrite /VCG.bidSum.
under eq_bigr => i _. rewrite tnth_map ffunE tnth_ord_tuple; over.
under [in leqRHS]eq_bigr => i _. rewrite tnth_map ffunE tnth_ord_tuple; over.
...
Definition Ri_b (a1s : A1s) (a2s : A2s): Prop :=
  (forall i, Ra i (action_on a1s i) (action_on a2s i)) /\ 
    let: o2 := O2_outcome_b a2s in
    let: oS := VCG.oStars a1s in
    o2 \in oS.

Lemma fRiP_b (a2s : A2s) :  Ri_b (fRi a2s) a2s.
Proof.
split; first by move=> i o; rewrite /action_on tnth_map /fR ffunE tnth_ord_tuple.
by have o2f : O2_outcome_b a2s \in VCG.oStars (fRi a2s) by exact: o2_in_ofRi2_b.
Qed.

Lemma MR_b (a1s : A1s) (a2s : A2s) (ri_b : Ri_b a1s a2s) : 
  Ro (m1 (G_choice a1s (O2_outcome_b a2s)) a1s) (m2_b a2s).
move: ri_b => [acts o2in].
have Gc1:     
  let: o2 := O2_outcome_b a2s in
  let: oS := VCG.oStars a1s in
  ...
  move=> o2in'.
  by rewrite G_choice_op.
congr (_, _) => //; first by rewrite /O1_winners /O2_winners_b /= Gc1.
apply: eq_from_tnth => s.
rewrite !tnth_map/= /O1_winners.
congr (inord _).
Check eq_instance_VCG_price.
Admitted.

Definition U2_b := (@prefs.U A2 _ m2_b p2_b).

Lemma RelFP_b (a1s : A1s) (o1 : O1) (o2 : O2) : 
  Ro o1 o2 -> forall (i : A), (@U1 a1s o1) i o1 = @U2_b i o2.
Proof. 
move=> ro i.
rewrite /U1/U2_b /p1 /p2_b/=.
rewrite /Ro in ro.
by rewrite /v1 /v2 !ffunE !ro.
Qed.

Lemma search_breaking_truthful_rel : @partial_truthful _ _ xpredT _ p2_b.
Proof.
move=> [a2s g2] [a2s' g2'] i hd2 hv2.
have ho := MR_b (fRiP_b a2s).
have ho' := MR_b (fRiP_b a2s').
rewrite /Ro in ho ho'.
set g' := G_choice (fRi a2s') (O2_outcome_b a2s').
set G' := G_choice (fRi a2s') (m1 g' (fRi a2s')).
have eqGc' bs o : 
  (m1 (G_choice (fRi bs) o) (fRi bs)).1 = (m1 G' (fRi bs)).1.
  by admit.
(*
- pose oS := VCG.oStars (fRi bs); pose oSne0 := VCG.oStars_ne0 o10 (fRi bs).
  have o2in : sval (G_choice (fRi bs) o oSne0) \in oS.
  ...
  - exact: (@proj2_sig _ (fun x => x \in oS)).
  congr (_, _).
  rewrite /O1_winners/G'.

  apply: eq_from_tnth => s.
  rewrite !tnth_map/= /O1_winners/= /m1.
  rewrite /VCG.price.
  congr (inord (_ - _)).
  rewrite//=.
  rewrite/VCG.welfare_without_i/=/VCG.bidSum_i.
  apply: eq_bigr => j _.
*)

have MR' : ∀ (bs1 : n.-tuple A1) (bs2 : A2s),
    true -> mech.Ri Ra bs1 bs2 → Ro (m1 G' bs1) (m2_b bs2). 
  by admit.
(*
  move=> bs1 bs2 g1.
  have := @MR_b bs1 bs2. 
  ...
  by rewrite /mech.Ri -(Ri_ofRi mri).
*)
have RelFP' : ∀ (o1 : mech.O (m1 G')) (o2 : mech.O m2_b),
    Ro o1 o2 → (prefs.U (p1 G'))^~ o1 =1 (prefs.U p2_b)^~ o2.
- by move=> o1 o2 ro12; apply: RelFP_b; first by exists (fRi a2s); rewrite size_tuple.
apply: (partial_MP _ _ _ xpredT _ _ (p1 G') p2_b _ _ fRP fRvP _ MR' RelFP')=> //.
exact: truthful_General_VCG.
Admitted.

(* Indifference *)
Variable c2' : C2. (* c2 c2' *)
...
Definition outcomes2 a2s := VCG.outcomes o0 (fRi a2s).
...
Check VCG.truthful_breaking_indifferent2.

End Relationsbreaking.
...
Check (@VCG.truthful_breaking_indifferent2 _ S.o0).
