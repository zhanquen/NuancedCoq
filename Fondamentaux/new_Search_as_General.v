...
Lemma ltn_slot_of_k i o : slot_of i o < k.
Proof.
rewrite /slot_of.
by case: tnthP => [[w hw //]|_ //=].
Qed.
...
Lemma not_slot_in_oStar (j : A) :
  (j \notin (oStar bs)) -> slot_of j (oStar bs) = last_slot.
Proof.
move/negPf => jnotino.
rewrite /slot_of.
move: (@elimF _ _ (tnthP (oStar bs) j) jnotino).
case: tnthP => pj _ //=.
Qed.
...
(** VCG for Search algorithm adapted to tie-breaking.
The choice of breaking is parameterized.
*)
...
Let bs_k_sorted := [tuple tnth bs (slot_as_agent s)| s < k].

Definition oStars :=
  [set o : O | (map_tuple (tnth bs0) (obidders o)) == bs_k_sorted].

Definition OStar := {o : O | o \in oStars}.

Let oStar_ref := OStar.oStar bs0.

Notation "'a* s" := (tnth (tlabel bs0) (slot_as_agent s)) (at level 10).

Lemma oStar_ref_in : oStar_ref \in oStars.
Proof.
rewrite inE.
apply/eqP; apply: eq_from_tnth; rewrite/eqfun => s.
rewrite !tnth_map tnth_ord_tuple.
by rewrite -(labelling_spec_idxa bs0 ('a* s)) (cancel_inv_idxa bs0).
Qed.
...
Proof.
apply/set0Pn; exists oStar_ref; exact: oStar_ref_in. 
Qed.
...
(* similar to stable_choice and stable_choice_in *)
Definition in_VCG_choice : VCG.OStar_choice O_finType :=
  fun (s : {set O_finType}) (s0 : s != set0) => 
    match inspect (oStar_chosen \in s) with
    | exist true oin => @exist _ (fun o => o \in s) oStar_chosen oin
    | exist false _ => OStar.set_pick s0
    end.

Lemma in_VCG_choice_in (s : {set O_finType}) (s0 : s != set0) :
  oStar_chosen \in s -> sval (in_VCG_choice s0) = oStar_chosen.
Proof.
move=> oin.
rewrite /in_VCG_choice.
case: (inspect (oStar_chosen \in s)).
by case=> p //=; last by rewrite p in oin.
Qed.

Section Welfarebreakings.

(* The oStar chosen maximizes social welfare *)

Lemma eq_welfare_chosen_ref :
  OStar.welfare bs0 oStar_chosen = OStar.max_welfare bs0.
Proof.
apply: eq_bigr => s _.
...
rewrite !tnth_map tnth_ord_tuple.
move=> ->.
by rewrite -(labelling_spec_idxa bs0) (cancel_inv_idxa bs0).
Qed.

Lemma le_welfare_o_oStar (o : O) : 
  OStar.welfare bs0 o <= OStar.welfare bs0 oStar_chosen.
Proof.
rewrite eq_welfare_chosen_ref.
exact: OStar.le_welfare_o_oStar.
Qed.

End Welfarebreakings.

Variable (i : A).

Let i' := slot_of i oStar_chosen.
...
Lemma after_last_slot : k' <= i' -> 'ctr_(slot_won) = ctr0.
Proof.
move=> lek'i.
apply: val_inj => /=. 
...
Definition price:=
  if is_winner
    then \sum_(s < k | i'.+1 <= s) externality bs s 
  else 0.

End VCGforSearchAlgorithm.

(** Optimal outcome for sorted bids adapted to tie-breaking.
The reference choice of [0, k'] as winners is always optimal.
 *)
...
Let oStars := oStars bs.

Lemma t_oStar_in_oStars : oStar \in oStars.
Proof.
rewrite inE.
apply/eqP; apply: eq_from_tnth; rewrite/eqfun => s.
rewrite !tnth_map tnth_ord_tuple.
...
Variable (oc_ref : OStar_choice).

Let oStar_chosen := oStar_chosen bs oc_ref.

Hypothesis reference_choice : oStar_chosen = oStar.
...
Lemma le_welfare_o_t_oStar (o : O) : welfare bs o <= max_welfare.
...
Proof.
rewrite -!eq_oStar_iota -{3}(idxaK sorted_bs j); exact: OStar.slot_in_oStar. 
Qed.

Lemma not_slot_in_oStar (j : A) : (j \notin oStar) -> slot_of j oStar = last_slot.
Proof.
rewrite -!eq_oStar_iota.
by apply OStar.not_slot_in_oStar.
Qed.
...
Let slot_won := slot_won ^~ oc_ref.

Lemma wonE j (jwins : idxa bs j < k') : slot_won bs j = inord (idxa bs j).
Proof.
rewrite idxaK //; rewrite idxaK // in jwins.
apply: val_inj => /=.
rewrite -/oStar_chosen reference_choice.
rewrite slot_in_oStar.
- rewrite inordK; last by apply: leqW.
  by apply/minn_idPl/ltnW.
- apply: mem_oStar.
  by apply/ltnW.
Qed.
...
Variable (bs : bids) (i : A) (oc oc_ref : OStar_choice).
...
Let oStar_ch := oStar_chosen bs oc.
Let oStar_ref := oStar_chosen bs' oc_ref.
Hypothesis reference_choice : oStar_ref = oStar.

Definition label (o' : O) (j : A) := slot_as_agent (slot_of j o').

Lemma label_oStar_inv (o' : O) (j : A): 
  label oStar_ref (label o' j) = label o' j.
Proof.
rewrite/label reference_choice.
congr slot_as_agent.
(* (slot_of i oStar_ch) is of type 'I_k. *)
(* By applying mem_oStar, (slot_of i oStar_ch) is in oStar. *)
(* By applying slot_in_oStar,  the slot of (slot_of i oStar_ch) in oStar
is equivalent to inord_k (slot_of i oStar_ch).
*)
rewrite {1}(slot_in_oStar tsorted_bids_sorted); last first.
- apply: (mem_oStar tsorted_bids_sorted).
  rewrite/slot_as_agent.
  (* to deal with slot_as_agent (slot_of ... *)
  exact: (ltn_ord (slot_of _ _)).
- by rewrite inord_val.
Qed.

Lemma slot_of_oStar_inv (j : A) (o' : O) : 
  slot_of j o' = slot_of (label o' j) oStar_ref.
Proof.
rewrite reference_choice.
rewrite (slot_in_oStar tsorted_bids_sorted) /label; last first.
- apply (mem_oStar tsorted_bids_sorted); last by exact: (@ltn_ord k).
- by rewrite inord_val.
Qed.

Let i' := (label oStar_ch i).

Lemma eq_relabelled_price : 
  price bs' oc_ref i' = price bs oc i.
Proof.
rewrite /price /is_winner tsortK //; last by apply tsorted_bids_sorted.
by rewrite -/oStar_ref -slot_of_oStar_inv.
Qed.

(* instance_bidding specifies VCG.biddings in General.v *)
(* in instance_bidding, tie-breaking is parameterized to determine the slot of agent j *)

(*
Definition instance_bidding bs oc' (j : A) :=
  [ffun o' : O => t_bidding (tsort bs) (label (oStar_chosen bs oc') j) o'].

Definition instance_biddings bs oc' := [tuple instance_bidding bs oc' j | j < n].

Lemma relabelled_bidding j o': 
  instance_bidding bs' oc_ref (label oStar_ch j) o' = instance_bidding bs oc j o'.
Proof.
rewrite !ffunE /t_bidding.
rewrite -/bs' !(tsortK tsorted_bids_sorted).
by rewrite -/oStar_ref -/oStar_ch !label_oStar_inv.
Qed.

*)

Definition instance_bidding bs (j : A) :=
  [ffun o' : O => t_bidding (tsort bs) (label oStar_ref j) o'].

Definition instance_biddings bs := [tuple instance_bidding bs j | j < n].

Lemma relabelled_bidding j o': 
  instance_bidding bs' (label oStar_ref j) o' = instance_bidding bs j o'.
Proof.
rewrite !ffunE /t_bidding.
rewrite -/bs' !(tsortK tsorted_bids_sorted).
rewrite label_oStar_inv reference_choice.
by [].
Qed.

(*
Lemma jk_sorted_relabelled_bidding (j : A) o': 
  j < k -> instance_bidding bs' oc_ref j o' = bidding bs' j o'.
Proof.
rewrite !ffunE /t_bidding -/oStar_ref reference_choice (tsortK tsorted_bids_sorted).
rewrite /label.
move=> jk.
have jino := mem_oStar tsorted_bids_sorted jk.
rewrite (slot_in_oStar tsorted_bids_sorted jino).
have -> : slot_as_agent (inord j) = j.
  apply: val_inj => /=.
  by rewrite inordK.
by [].
Qed.

Lemma k'j_sorted_relabelled_bidding (j : A) o': 
  k' < j -> instance_bidding bs' oc_ref j o' = bidding bs' (slot_as_agent last_slot) o'.
Proof.
rewrite !ffunE /t_bidding -/oStar_ref reference_choice (tsortK tsorted_bids_sorted).
rewrite /label.
move=> k'j.
have jnotino := not_in_oStar tsorted_bids_sorted k'j.
rewrite (not_slot_in_oStar tsorted_bids_sorted); last by apply/negPf.
by [].
Qed.
*)
...
Let SS_price := price.
Let VCG_price := VCG.price o0.

Theorem eq_instance_VCG_price :
  SS_price bs oc i = VCG_price i (biddings bs) (in_VCG_choice bs oc).
Proof.
rewrite /SS_price /VCG_price.
...
Lemma mem_oStar (j : A) : slot_of j oStar_chosen < k' -> j \in oStar_chosen.
Proof.
rewrite/slot_of.
case: (tnthP oStar_chosen j)=> /=.
- by move=> p _.
- move=> _.
  by rewrite ltnn.
Qed.

Lemma mem_oStar_inv j : j \in oStar_chosen → slot_of j oStar_chosen < k.
Proof.
move/tnthP=> [ij] ->.
by rewrite/slot_of.
Qed.
...
(*
mem_oStar_equiv, not_in_oStar', not_in_oStar, not_in_oStar_inv, wonE
no longer defined
*)

Lemma eq_losing_last_slot j
      (jino : j \in oStar_chosen)
      (jloses : slot_of j oStar_chosen < k' = false) :
  slot_of j oStar_chosen = last_slot.
Proof.
move: (jino) => /mem_oStar_inv.
rewrite ltn_neqAle => /andP [_].
rewrite leq_eqVlt ltnS leq_eqVlt jloses orbF.
...
Notation "'bid_ j" := (tnth bs0 j) (at level 10).

Definition bidSum := @VCG.bidSum O_finType (biddings bs0).

Lemma bidSumE o : bidSum o = \sum_(b <- biddings bs0) b o.
Proof. 
by rewrite /bidSum VCG.bidSumE.
Qed.

Lemma valid_bidSum (o : O) :
  bidSum o = \sum_(j < n) (bidding bs0) j o.
Proof. by apply: congr_big => //= s _; rewrite tnth_mktuple. Qed.

Lemma bidSum_out_o (o : O) :
  \sum_(j < n | j \notin o) (bidding bs0) j o = 0.
Proof.
apply/eqP; rewrite sum_nat_eq0.
apply/forall_inP => j outo; apply/eqP.
rewrite ffunE.
...
Lemma bidSum_slot (o : O) :
  bidSum o = \sum_(s < k) (bidding bs0) (tnth o s) o.
Proof.
rewrite valid_bidSum (bigID (fun j : A => j \in o)) bidSum_out_o /= addn0.
by rewrite -(@big_tuple _ _ _ _ _ _ predT (fun j => bidding bs0 j o)) big_uniq ?ouniq.
Qed.

Notation "'a* s" := (tnth oStar_chosen s) (at level 10).

Notation "'b* s" := (bidding bs0 ('a* s) oStar_chosen) (at level 10).

Lemma bidsSum_sumBid (P : A -> bool) (o : O) :
  forall bs, \sum_(j < n | P j) tnth (biddings bs) j o = \sum_(j < n | P j) bidding bs j o.
Proof. by move=> ?; apply: congr_big => //= j _; rewrite tnth_mktuple. Qed.

Definition VCG_oStars := @VCG.oStars O_finType (biddings bs0).

Lemma HoStar : oStar_chosen \in VCG_oStars.
Proof.
rewrite in_set; apply/andP; split; first by [].
apply/forallP=> x; rewrite implyTb.
rewrite /VCG.bidSum !bidsSum_sumBid -!valid_bidSum !bidSum_slot.
...
Lemma oStar_extremum : max_bidSum_spec oStar_chosen.
Proof.
apply: ExtremumSpec => [//|o _].
rewrite bidSum_slot /bidSum /VCG.bidSum // !bidsSum_sumBid.
move: (le_welfare_o_oStar o).
by rewrite /OStar.max_welfare /OStar.welfare -bidSum_slot valid_bidSum.
Qed.

(*
bid_ctr_slot eq_bid_ctr bidSum_i_bid_ctr_slot
no longer defined
*)

Definition bid_ctr_agent j := 'bid_j * 'ctr_(slot_of j oStar_chosen).

Lemma widen_slot_as_agent (s : slot) : widen_ord le_k_n s = slot_as_agent s.
by rewrite /slot_as_agent; apply: val_inj.
Qed.

Definition bidSum_i := @VCG.bidSum_i O_finType i (biddings bs0).

Lemma bidSum_i_bid_ctr_agent : bidSum_i oStar_chosen = bidSum oStar_chosen - bid_ctr_agent i.
Proof.
rewrite /bidSum_i /VCG.bidSum_i /bidSum /VCG.bidSum.
rewrite [X in _ = X - _](bigD1 i) => //=.
rewrite addnC -[LHS]addn0 -addnBA.
congr (_ + _).
- rewrite tnth_mktuple ffunE /t_bidding.
  case: (i \in oStar_chosen); first by rewrite subnn.
  by rewrite sub0n.
- rewrite tnth_mktuple ffunE /t_bidding.
  case iino: (i \in oStar_chosen); first by apply: eq_leq.
  rewrite /bid_ctr_agent /slot_of.
  move: (@elimF _ (i \in oStar_chosen) (tnthP oStar_chosen i) iino).
  case: tnthP => [p|p _] //.
  by rewrite leqn0 muln_eq0 last_ctr_eq0 ?eq_refl ?orbT.
Qed.
...
Definition t_bidding_i j o := if j != i then t_bidding bs0 j o else 0.
...
(*
sum_out_perm_i no longer defined
*)
...
Variable (bs : bids) (i : A) (oc : OStar_choice).

Let bs' := tsort bs.
Let i' := slot_as_agent (i' bs oc i).

Lemma tsorted_bids_sorted : sorted_bids bs'.
Proof. by apply/sorted_bids_sorted/sort_sorted/total_geq_bid. Qed.

Definition oStar_iota := Outcome oStar_uniq.

Definition o0 := oStar_iota.

Definition oStars_iota := [set oStar_iota].

Let oStar := oStar_chosen bs oc.

Let stable_oc := stable_choice bs oc.

Let SS_price := price.

Let G_price := VCG.price o0.

Theorem eq_instance_VCG_price : 
  SS_price bs oc i = G_price i (biddings bs) stable_oc.
Proof.
rewrite/SS_price/G_price.
rewrite/price/Search_as_General.price/VCG.price.
have [iwins | iloses] := boolP (is_winner bs oc i).
- by admit.
  rewrite VCG.eq_welfare''.

all:  /VCG.welfare_without_i''.
all: rewrite/VCG.welfare_with_i.
all: rewrite -VCG.eq_bidSum''.
- by admit.
rewrite/VCG.welfare_without_i.
Admitted.
...
Lemma slot_in_oStar (j : A) : (j \in oStar) -> slot_of j oStar = inord j.
Proof. rewrite -!eq_oStar_iota -{3}( sorted_bs j); exact: OStar.slot_in_oStar. Qed.

Lemma mem_oStar_inv (j : A) : j \in oStar -> j < k.
Proof. 
rewrite -eq_oStar_iota -{2}( sorted_bs j).
exact: OStar.mem_oStar_inv.
Qed.
...
Proof.
move: jisslot.
rewrite -eq_oStar_iota -{1}( sorted_bs j).
exact: OStar.mem_oStar.
Qed.
...
Proof.
rewrite /price /is_winner tsortK ? // /price /externality VCG.eq_price'' /VCG.price''.
...
Lemma eq_sorted_VCG_price_wins :  
  price bs i = @VCG.price O_finType o0 i (biddings bs) (stable_choice bs).
Proof.
rewrite /price /is_winner tsortK ? //. 
...
rewrite  //; last exact: tsorted_bids_sorted.
Qed.
...
Proof.
rewrite !ffunE /t_bidding.
by rewrite !( tsorted_bids_sorted) !(tsortK tsorted_bids_sorted).
Qed.
...
Section VCGforSearchPriceBreaking.

Variable (bs : bids) (i : A) (oStar_choice : OStar.OStar_choice).

Let oc := OStar.oStar_chosen oStar_choice bs.
(*
Variable (SSoStar_choice : OStar.OStar_choice) 
  (GoStar_choice : VCG.OStar_choice O_finType).
*)

Let SSoStars := OStar.oStars bs.
Let GoStars := VCG.oStars (instance_biddings bs).

(*
Lemma eq_set_oStars : SSoStars = GoStars.
Proof.

apply/subsetP; rewrite /sub_mem=> o Hoin.
- rewrite inE andTb. 
  apply/forallP => o'.
  apply/implyP => _.
...
Check (OStar.price_chosen oStar_choice bs i).

Print instance_bidding.

Definition instance_bidding_b bs (j : A) :=
  [ffun o' : O => t_bidding bs j o'].

Definition instance_biddings_b bs := [tuple instance_bidding_b bs j | j < n].

Lemma relabelled_bidding_b j o': instance_bidding_b bs j o' = bidding bs j o'.
Proof.
rewrite !ffunE /t_bidding. 
by congr (if _ then _ else _).
Qed. 

Lemma relabelled_biddings_b: instance_biddings_b bs = biddings bs.
Proof.
apply: eq_from_tnth => j.
rewrite !tnth_mktuple.
by apply/ffunP => o'.
...
Let welfare_without_i'' := VCG.welfare_without_i'' i (biddings bs).

Lemma eq_welfare_without_i''_bidSum' : welfare_without_i'' = bidSum' oc.
Proof.
rewrite /welfare_without_i''/VCG.welfare_without_i''/bidSum'.
rewrite (@bigmax_eq_arg _ o0 _ (fun o => VCG.bidSum'' i (biddings bs) o))//.
Admitted.
...
rewrite bidsSum_sumBid -valid_bidSum bidSum_slot.
apply: eq_bigr => j _.
rewrite /bidding ffunE /bidding /t_bidding /bid_ctr_slot.
- have [] := boolP (tnth oc j \in oc) => jin.  
  congr (tnth bs _ * 'ctr_ _).

; first by exact: widen_slot_as_agent.
  rewrite (@slot_in_oStar bs0) //.
  apply: val_inj => /=.
  by rewrite inordK.
- have/not_in_oStar_inv: (widen_ord le_k_n j \in oStar_i) = false. 
...
Admitted.
Theorem eq_instance_VCG_price_b : OStar.price_chosen oStar_choice bs i = 
  VCG.price o0 i (instance_biddings_b bs) stable_choice_b.
Proof.
have [iwins | iloses] := boolP (OStar.is_winner_chosen oStar_choice bs i).
all: rewrite relabelled_biddings_b.
- rewrite/OStar.price_chosen iwins /externality.
  rewrite  VCG.eq_price'' /VCG.price''.
Print welfare_without_i''.
